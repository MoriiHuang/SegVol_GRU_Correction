...
import glob
import numpy as np
from segvol.segvol_gru_correction import build_overlap_mask, build_binary_cube_dict, SegVolModel
...
class SegDatasetTrain(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(unique_labs) == 0:
            # 处理方式1：跳过这个数据
            # raise ValueError(f"Empty unique_labs for index {idx}")
            return None
            # 处理方式2：默认一个标签
            # unique_labs = [default_label]
        selected_lab = unique_labs[self.epoch % len(unique_labs)] 
...
def train_one_epoch(model, train_dataloader, optimizer, epoch,stage='box'):
    model.train()
    total_loss = 0
    with tqdm(train_dataloader, desc=f'Epoch {epoch}') as pbar:
        for step, data_item in enumerate(pbar):
            ...
            try:
                ...
                print('some error might happen in the data')
                continue
            loss = model(
...
if __name__ == '__main__':
    ...
    initial_lr = 1e-5
    eta_min = 1e-6
    train_root_path = '/home/sjtu-huang/CVPR2025/3D_train_npz_all'#'/home/sjtu-huang/CVPR2025/3D_train_npz_random_10percent_16G' #
    resume_checkpoint = '/home/sjtu-huang/SegVol-for-SegFM-main/epoch_2000_loss_0.2232.pth'
    # resume_checkpoint = './SegVol_v1.pth'
    save_dir = './ckpts_fm3d_segvol_0522'
    ###########################
    model_dir = './segvol'
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if local_rank == 0:
            ...
            print(f'Current learning rate: {current_lr:.10f}, Stage: {stage}')

        avg_loss = train_one_epoch(model, train_dataloader, optimizer, epoch, stage=stage)

        if local_rank == 0:
            writer.add_scalar(f'train/{stage}_avg_loss', avg_loss, epoch)

            # 检查是否切换stage
            if prev_loss is not None and abs(prev_loss - avg_loss) < 0.05:
                if stage == 'box':
                    print("Switching stage: 'box' → 'box_points'")
                    stage = 'box_points'
            prev_loss = avg_loss
