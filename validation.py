...
class SegDatasetTest(torch.utils.data.Dataset):
    def __init__(self, file_path_pairs, preprocessor):
        """
        file_path_pairs: List of [img_npz_path, gt_npz_path]
        preprocessor: your SegVolProcessor 之类的实例
        """
        self.img_paths = [p[0] for p in file_path_pairs]
        self.gt_paths  = [p[1] for p in file_path_pairs]
    ...
    def __getitem__(self, idx):
        ...
        # 第一份 npz：imgs + boxes
        npz_img = np.load(img_path, allow_pickle=True)
        imgs    = npz_img['imgs']      # e.g. (C,H,W,D) or (1,H,W,D)
        boxes   = npz_img['boxes']     # list of std_box

        # 第二份 npz：gts
        npz_gt  = np.load(gt_path, allow_pickle=True)
        gts     = npz_gt['gts']        # (N,H,W,D)

        # 先做 CT / GT 的统一预处理
        imgs, gts = self.preprocessor.preprocess_ct_gt(imgs, gts)

        # 再把每个 std_box 转成二值 cube
        cube_list = [
            build_binary_cube_dict(std_box, imgs.shape[1:]) 
            for std_box in boxes
        ]
        cube_boxes = torch.stack(cube_list, dim=0)  # (N, H, W, D)
        assert cube_boxes.shape == gts.shape, (
            f"{cube_boxes.shape} != {gts.shape}"
        )

        # 最后做 zoom_transform，并把 file_path 也带回去
        zoom_item = self.preprocessor.zoom_transform(imgs, gts, cube_boxes)
        zoom_item['img_path'] = img_path
        zoom_item['gt_path']  = gt_path
        return zoom_item
